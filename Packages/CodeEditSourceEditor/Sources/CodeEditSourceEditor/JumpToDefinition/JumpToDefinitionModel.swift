...
final class JumpToDefinitionModel {
    ...
    private func openLocalLink(link: JumpToDefinitionLink) {
        print("🏠 openLocalLink called for range: \(link.targetRange)")
        guard let controller = controller else {
            print("⚠️ openLocalLink: controller is nil")
            return
        }
        
        // Dùng setCursorPositions với scrollToVisible để di chuyển cursor và scroll tới đúng vị trí.
        // Cần gọi trên main thread vì đây là UI update.
        let position = link.targetRange
        print("📍 openLocalLink: Jumping to line=\(position.start.line) col=\(position.start.column)")
        
        // Đảm bảo textView nhận focus trước, sau đó nhảy tới vị trí
        controller.textView.window?.makeKeyAndOrderFront(nil)
        controller.textView.window?.makeFirstResponder(controller.textView)
        
        controller.setCursorPositions([position], scrollToVisible: true)
    }
