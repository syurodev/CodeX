...
extension TextViewController {
    ...
    public func resolveCursorPosition(_ position: CursorPosition) -> CursorPosition? {
        ...
        if range == .notFound {
            ...
            if position.end != nil {
                range = NSRange(
                    location: linePosition.range.location + position.start.column - 1,
                    length: linePosition.range.length
                )
            } else {
                range = NSRange(location: linePosition.range.location + position.start.column - 1, length: 0)
            }
        }
